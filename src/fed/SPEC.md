...
B4|2026-08-01|`check_edge_depth` (LLM-authored, 2nd run) re-scans the `§F` section itself — its own `in_f` loop & header skip — instead of calling `edges(text)` & checking `e.dir`. residual two-readings, milder than B1's duplicate walker but real|call `edges()`. step 2 sees `edges` in its surface ∴ cause is the same as B1's: nothing ASKS for reuse, & asking made it worse (`.:tdd` B?)
