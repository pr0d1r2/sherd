...
B13|2026-09-19|`nav_section()` wrote the lens RAW while `split_row()` had UNESCAPED it ∴ an `owns` cell holding `\|` came back as `\|` & every `§N` row naming that node gained a 4th column — this repo carried it in 17 `§N` files, every one naming `src/lens` ('s ``depth `rule`\|`why` ``). the V4 tests covered PARSING only; nothing rendered a parsed cell & read it back\|V16: `escape_cell()` in `nav_section()`. GENERALLY: a codec is verified by its ROUND TRIP — `a_lens_holding_a_pipe_stays_one_cell` pushes `|`, `C:\path`, a trailing `\` & `\|` through render then parse
