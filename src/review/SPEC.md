...
T7|.|a shape mined from a RECURRENCE, ⊥ a transform: `src/cli:B6`·`B7`·`B8` are 1 defect 3 times — a verb reading an OPTIONAL artefact ! tell ABSENT from MALFORMED from EMPTY, & each decided alone. detectable? `is_file()` guard vs a bare read on a path the verb also treats as optional|V9,`.:V121`
