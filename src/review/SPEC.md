...
V10: ⊥ every RECURRENCE yields a SHAPE. `src/cli:B6`·`B7`·`B8` are 1 defect 3 times — a verb reading an OPTIONAL artefact ! tell ABSENT from MALFORMED from EMPTY — & a detector for it was PROBED & REFUTED before being written: "reads a dotfile & has no `is_file()`" fired 7 times on the FIXED tree & once on the buggy one, exactly backwards. the 7 were a message STRING naming `.lint-debt`, an `.ok()?` that IS the guard, & an `is_cold()` that is the guard in another form ∴ the guard has ≥5 valid spellings (`is_file`·`exists`·`Err(_) =>`·`.ok()?`·a `Result` handed to a caller who decides) & "reads it" ⊥ separable from "mentions it". the distinction is SEMANTIC, ⊥ textual. a shape ships only w/ a measured hit rate ∵ `V3` makes findings advisory & an advisory that is wrong 7 of 7 times trains a reader to skip the whole report
