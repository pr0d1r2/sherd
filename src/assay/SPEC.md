...
B3|2026-08-24|`a_test_that_will_not_compile_is_not_a_correct_rejection` FAILED once in the gate & passes alone, twice in the full suite, & on the tree without the change ∴ a FLAKE, ⊥ a regression. cause: it shells out to `rustc` under `GRADE_TIMEOUT` 10s, & that run happened DURING a cold `--all-features` rebuild — `ring`, `rustls` compiling on every core — where the suite took 60.27s against ~10s idle. the const's own doc says "~10x headroom rather than a tuned number", & the headroom was measured on an IDLE machine ∴ it is a claim about THIS machine doing nothing else. RECORDED, ⊥ fixed: `.:V117` freezes this node until rung `0.7`, & a `§B` row is a record rather than development|`.:V119`-adjacent, `src/ollama:B8`. a flapping gate is WORSE than a slow one — B8 is what one cost: "I committed through the red gate TWICE rather than reading it, which is the `--no-verify` reflex wearing a different hat", & I reached for `--no-verify` on THIS one before the classifier refused it. GENERALLY: a TIMEOUT tuned on an idle machine is `.:B19` on another axis — a default measured in one condition, asserted over all of them — & the fix is ⊥ a bigger number but a bound that scales w/ what else is running, or a suite that ⊥ shell out under a gate
