...
T13|x|point `BBX_STATE` at a temp file per test so the suite stops sharing `.bbx-state`. the 0.3 coverage margin in `hk.pkl` is a WORKAROUND for this & should be removed w/ it|V19
