...
///
/// The CODE dag is NOT the federation dag (`V23`). These edges come from
...
pub struct Schedule {
    ...
    /// looped over, and not a defect (`V23`).
...
pub fn code_deps(root: &Path) -> Vec<CodeDep> {
    let nodes = crate::fed::discover(root);
    nodes
        .iter()
        .map(|n| CodeDep {
            node: crate::fed::node_label(root, n),
...
fn needs_of(root: &Path, node: &Path, nodes: &[PathBuf]) -> Vec<String> {
    ...
    let mut out: Vec<String> = owned_sources(node, nodes)
        .iter()
        .flat_map(|t| crate::code::crate_uses(t))
...
fn siblings_of(
    ...
    nodes
        ...
        .filter_map(|n| {
            ...
            Some((name, crate::fed::node_label(root, n)))
        })
        .collect()
}
...
fn owned_sources(node: &Path, nodes: &[PathBuf]) -> Vec<String> {
    crate::fed::owned_rust_files(node, nodes)
        .iter()
...
mod wave_tests {
    ...
    fn code_dag_fixture() -> Result<crate::testrepo::TestRepo, String> {
        let r = crate::testrepo::TestRepo::new("plan-wave")?;
    ...
    /// `V23`, from the other side: `§F` makes `a` and `b` CO-CHILDREN with no
    ...
    -> Result<(), String> {
        ...
        let children: Vec<String> = crate::fed::edges(&src)
            .iter()
            .map(|e| e.dir.clone())
            .collect();
        assert_eq!(
