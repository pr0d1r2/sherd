...
B8|2026-08-24|`budget` & `validate` FAILED a stranger against a ceiling nobody wrote. ashlar has no `.context-limits` ∴ every node took `DEFAULT_NODE` 2,000 & the report read `chain 7484 · ceiling 2000 · OVER by 5484`, exit 1, w/ nothing saying the file was ABSENT or the number a default. `Ceilings::load`'s own doc has said "a missing file is a COLD START" since it was written — the loader knew & the CALLER could ⊥ ask. 4th instance of 1 shape today (`B6` `fed`+`slice`, `B7` `debt`, this): every verb resolves its own ambient facts & each decides absence alone|V12. `Ceilings::is_cold()`; over a cold default is REPORTED (`.:V48`) & ⊥ counted against the verdict. an unlisted PATH inside an EXISTING file still takes the default — `.:V6` requires it, or a row silently skipped gates nothing — & the test pins cold vs warm vs unlisted-in-warm. GENERALLY: a DEFAULT is a suggestion until someone writes the file it lives in, & a gate that cannot tell "unset" from "set to this" fails people for rules they never adopted
B9|2026-09-19|`root_for` scanned argv for the 1st `p.is_dir()` & handed THAT PATH — possibly relative — to `repo_root_from`, whose walk climbs by `parent()` to `""`, which holds no `.git` ∴ the walk returned its own START & the ARGUMENT became the root; `arg_dir` then joined the argument on again. MEASURED: `seam src` @ root exit 2 `src matched no node` · `seam code` @ `src/` exit 2 `code/code matched no node` · `seam ./code` → `./code/./code` — the DOUBLING is the proof. `./x` resolved only from the repo ROOT ∴ "use `./`" was ⊥ a workaround, just the 1 cwd where both readings agree. ∀ `[dir]` verb: budget·seam·lens·check·fed. 3rd of `B5`/`B7`'s family & ⊥ caught by either, ∵ both were tested w/ ABSOLUTE fixture paths only. NOTE this is `src/cli:B9`, ⊥ `.:B9`|V15. resolve the candidate against the CWD before the walk; `root_for_from(args,cwd)` takes the dir a cmd was typed in ∴ a test is HANDED it (V6) ⊥ mutating the process CWD. LEXICAL `join` ⊥ `canonicalize`: canonicalize rewrites symlinks (macOS `/var/folders` → `/private/var`) ∴ the root would stop comparing equal to the path it was given & 2 existing tests would need weakening. RESIDUE: `arg_dir` is ROOT-relative BY DESIGN & pinned by a test ∴ `seam code` @ `src/` now names an honest absolute path & still exits 2 — the root-relative `seam src/code` is the spelling that works
B10|2026-09-20|`sherd --version` & `-V` fell through to `Some(other)` ∴ both printed the USAGE banner on STDERR & exited 2 — the code V1 reserves for a MALFORMED invocation — so there was no way to ask the binary what it is, & asking a tool its version is ⊥ a usage error. MEASURED on the flake build: `sherd --version; echo exit=$?` → banner, `exit=2`, same for `-V`. every sibling in the toolchain answers (`microlith 0.7.0`, `itok 0.3.1`, `hk 1.58.1`) ∴ a CI gate recording the version of every tool it ran could record all of them BUT ours, & a wrapper's only recourse was parsing the banner or shelling out to the package manager|V16. a dispatch arm beside `--help`'s printing `sherd {VERSION}`, `VERSION` being `env!("CARGO_PKG_VERSION")` ∴ it cannot drift from the manifest, & the test reads the manifest rather than restating the number. `dev`'s verb check excludes it as it already excludes `--help`: a FLAG answering ABOUT the binary is ⊥ a verb & belongs in no Commands table. GENERALLY: the fall-through arm decides what an UNRECOGNISED argument means, & the conventions the world already has for one — `--version`, `--help` — are exactly the cases it gets wrong by default
