...
B4|2026-09-19|`declares()` matched only a line OPENING w/ the id, while `citation_at` accepts `C` & `I` ids ∴ `` `NODE:C1` `` pointing at a real `- C1:` bullet was reported `resolves to no row` — the parser took a citation `check` could never resolve, & bare `C` ids went unchecked ∴ constraints were uncitable across nodes. found splitting a monolith's `§C` across nodes: every moved constraint became uncitable from its old neighbours|V8: `declares()` reads past a leading `- `. GENERALLY: the grammar a PARSER accepts & the grammar a RESOLVER declares ! be the same set — `a_bullet_declares_the_id_it_opens_with` pins both `C` & `I`
