...
V9: a MILESTONE RANGE (`T1-T3`) the map would SPLIT is refused UP FRONT & the cell is quoted, ⊥ expanded here: which node keeps the row once its ids land in 2 is a READER's judgement (V2's rule, 1 column over). the citation rewrite works token-by-token ∴ a range becomes `` `CHILD:T1` ``-`T3` & `check` then reports a SYMPTOM 3 steps from the cause (B5). a range NOBODY moves is left alone, or V6's rerun fails forever
...
B5|2026-09-20|`adopt --map` over a monolith whose milestone row lists a RANGE — `\| M1 \| … \| T1-T3 \|`, the form `microlith/V15` documents as the CHEAP way to maintain the column ∴ the form a brownfield spec most likely carries — wrote NOTHING & refused w/ `microlith/V15: T3 is in no milestone`. the rewrite that namespaces moved ids walks TOKEN by token ∴ `T1-T3` became `` `CHILD:T1` ``-`T3`, which expands to NO tasks, ∴ the last root row is claimed by no milestone & THAT is what got reported — a symptom 3 steps from the cause, naming a row that never moved. MEASURED on a 3-row fixture (sherd 0.5.0, microlith 0.7.0): range → rc=1 & 0 files; the same repro w/ `T1, T2, T3` → 3 moved, 2 files written ∴ the shape most likely to be adopted was the 1 shape adopt refused, & for a reason its own message did ⊥ name|V9 & `src/spec:V10`: the gap between what a row CLAIMS & what it SAYS detects the range w/o re-reading the range grammar, & the refusal QUOTES the cell. ⊥ expanded automatically: the ids of 1 milestone may land in 2 nodes & which keeps the row — or whether the table leaves `SPEC.md` entirely, as this repo's own reporter did — is a reader's call. GENERALLY: a rewrite that walks TOKENS silently breaks every construct built from more than one, & the checker downstream reports where the damage SHOWS, ⊥ where it was done
