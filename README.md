...
Generated by `bbx graph` from the `§F` tables — never hand-drawn, so it
cannot drift from what the specs declare.

```mermaid
flowchart TD
    root["."] --> src["src<br/><i>code nodes — tokens, spec, fed, lens facades …</i>"]
    src["src"] --> src_tokens["tokens<br/><i>'itok' facade, counts w/ method label, entry …</i>"]
    src["src"] --> src_spec["spec<br/><i>'cavespec' facade, §-section split, structura…</i>"]
    src["src"] --> src_fed["fed<br/><i>'§F' parse, edges, chain root→node, 'SPEC.md'…</i>"]
    src["src"] --> src_lens["lens<br/><i>pack assembly, depth 'rule'/'why', budget ver…</i>"]
    src["src"] --> src_ollama["ollama<br/><i>local endpoint client, 'num_ctx', fence extra…</i>"]
    src["src"] --> src_tdd["tdd<br/><i>red→judge→green→gate→repair loop, source regi…</i>"]```
