...
cannot drift from what the specs declare. Plain ASCII, nodes declared once,
no HTML: GitLab renders mermaid with `htmlLabels` disabled.

```mermaid
flowchart TD
    root["."]
    src["src: code nodes tokens, spec, fed, lens fa..."]
    src_tokens["tokens: itok facade, counts w/ method label, ..."]
    src_spec["spec: cavespec facade, S-section split, str..."]
    src_fed["fed: SF parse, edges, chain root-node, SPE..."]
    src_lens["lens: pack assembly, depth rule / why , bud..."]
    src_ollama["ollama: local endpoint client, num_ctx , fenc..."]
    src_tdd["tdd: red-judge-green-gate-repair loop, sou..."]
    root --> src
    src --> src_tokens
    src --> src_spec
    src --> src_fed
