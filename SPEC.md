...
T96|.|move the `#[ignore]`d titrations from `src/assay`'s inline tests to `tests/` — an ignored body counts in the coverage denominator & never executes, so every experiment added LOWERS the number & a better instrument reads worse (.coverage records the 75.50→75.11 drop). they are integration-level experiments & belong there anyway|V16,R50
