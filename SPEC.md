...
T82|.|re-run the generation titration w/ the task shape `bbx tdd` ACTUALLY uses (node context, `§V` id, author a test first) ⊥ a fixed signature. R44 measured the easy shape; T13's zero was measured on the hard one & they are ⊥ comparable|V106,R44
