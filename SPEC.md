...
V121: agentic debt resolution splits in 2 & only 1 half is a PIPELINE. the MECHANICAL half has a named transform ∴ detect → name the SHAPE → hand the model both → gate → repair, & every stage already exists (`src/review` detects, `src/debt` measures, `src/tdd` drives, `src/land` gates). the JUDGEMENT half does ⊥: `src/cli` B5·B6·B7·B8 — wrong root, absence-as-success, a position-shaped fix, a default read as a gate — are DESIGN defects, no catalogue detects them, & all 4 came from pointing the tool at a stranger & READING. MEASURED ceiling on the pipeline half: of 265 warnings, 111 are `too_many_lines` (a judgement over SEAMS, ⊥ a transform) & 47 arithmetic that is correct code ∴ the automatable residue is the indexing/duplication/status family, ~40-60 sites. the shapes are worth mining as a BY-PRODUCT of mechanical work; building the catalogue AS A PROJECT on the presumption is `.:B24` one level up — run what exists & measure what it leaves, before building
...
B24|2026-08-24|we have hand-applied ONE transform 34 times across 2 commits (`len()` check + positional index → slice pattern) & a 2nd that removed 24 index sites in 1 commit, & NOTHING recorded either ∴ the 3rd author to meet the shape re-derives it. asked whether AST "cops" w/ autocorrect were the answer & MEASURED the premise 1st: `cargo clippy --fix --workspace --all-targets --all-features` changes NOTHING on this tree — 252 warnings before & after, 0 diff — ∵ every warning we carry is marked ⊥ machine-applicable. the autocorrecting cop framework already SHIPS & our residue is exactly the half it declines|`src/review:V9`. `fix_shapes` NAMES 3 mined shapes & applies none. EVIDENCE that naming is the right half: 2 of my own first attempts at these rewrites were REFUSED — `hits.first().map_or(..)` by clippy's `explicit_auto_deref`, `d.first().map(|x| x.rule)` by `E0507` — ∴ the correct rewrite needs a TYPE, which means rustc, ⊥ a text scanner. our hand-rolled `fn_body` also broke twice in 1 hour (char literal `'{'`, lifetime `<'a>`) ∴ ⊥ hand-roll more parsing. GENERALLY: before building a tool, run the one that exists & measure what it leaves
B25|2026-08-30|the SUITE committed to the repo it was running in. git exports an ABSOLUTE `GIT_DIR` to hooks when the checkout is a WORKTREE (unset in a normal one) & every child inherits it ∴ `cargo test` under the pre-commit hook handed `TestRepo` a `GIT_DIR` aimed at the real repository. `current_dir` sets where git RUNS, ⊥ which repo it ACTS ON — env beats discovery ∴ `init`+`add -A`+`commit -m root` landed a commit deleting 90 files & leaving the fixture `SPEC.md`. invisible on `main` ∵ there `GIT_DIR` is unset|`TestRepo::command` env_removes the 8 exported `GIT_*`; 2 regression tests. GENERALLY: a fixture that shells out ! clear the env its runner exports — cwd is ⊥ isolation
B26|2026-08-30|the DEFAULT build warned & every gate step read GREEN. `f6793df` emptied `default` ∴ `apply` (`ollama`-gated) stopped compiling by default & took `preflight` — its only non-test caller — with it, while `run_args`'s `mut` existed only for the gated `-v` removal. 2 warnings rode through 11 gate steps, `cargo release hook`, the published `.crate` AND its verify. `clippy` denies warnings but runs `--all-features` ∴ never compiles this config; `no-default-features` compiles it & accepted warnings ∴ the shipping config was the one nothing held to a standard|`preflight` + its 3 tests gated w/ `apply`; `mut` shadowed under the feature. `no-default-features` gains `RUSTFLAGS=-D warnings` — safe only ∵ V101 made `itok` a REGISTRY dep & cargo caps lints for those, ⊥ for path deps. GENERALLY: a feature split ! be followed by asking which config the GATE actually compiles
B27|2026-08-31|`cargo release push` could ⊥ push a TAG ∴ the release stopped 1 step from done, twice. git hands `pre-push` `<local ref> <local sha> ...` & for a tag that sha is the ANNOTATED TAG OBJECT, ⊥ a commit; hk 1.55.0 feeds it to merge-base w/o peeling → `object is no commit object` @ hk `src/git.rs:1678`. our hook `exec`s hk on EVERY ref shape ∴ inherited the crash. workaround was `--no-verify`, which the hook's own text forbids — & the gate ⊥ REFUSED here, it CRASHED (`src/tdd:V26`: a gate that ⊥ RUN is an ERROR, ⊥ a breach)|hook peels `^{commit}` & SKIPS only when every pushed ref is a tag ALREADY reachable from `origin/main` — that commit passed this hook & CI already; a tag is a pointer to it. a branch ref, or a tag OFF trunk, still pays the gate. 3 cases MEASURED before trusting it. GENERALLY: a hook that `exec`s one tool on every ref shape inherits that tool's ref-shape bugs
