...
T90|.|coverage floor, RATCHETED ⊥ set: start AT the measured 64.76% (R50), rises only w/ the tests that earn it, `.coverage` holds the number like `.lint-debt` holds the other. 98% is the DESTINATION in §C, ⊥ a gate that goes red tomorrow — the clippy adoption already showed a cliff is unpayable & a ratchet catches real defects (3 so far)|V16,R50,B6
